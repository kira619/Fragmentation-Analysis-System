...
def mancon_lazzo(img_path,blur_val,cann,size,dst_val,factor,scale):
	global fine_cnts,cnts
...
def mancon_undo(img_path,blur_val,cann,size,dst_val,factor,scale):
	global fine_cnts,cnts
...
def mancon(img_path,blur_val,cann,size,dst_val,factor,scale):
	global fine_cnts,cnts
...
def fine_tune(path,factor,scale):
	...
	process_one_image(0,file_path,luke_skywalker.factor,int(e2.get()))
...
def luke_skywalker():
    ...
    scale = int(e2.get())
    luke_skywalker.factor = length(file_path,acc_ball,scale)
    show_page(page2)
...
def C3PO():# fine tune parameters for auto contouring
    ...
    acc_ball = float(e1.get())
    scale = int(e2.get())
    C3PO.blurr_val,C3PO.canny,C3PO.size,C3PO.dst_val = fine_tune(file_path,luke_skywalker.factor,scale)
...
def yoda():
    ...
    yoda.diameter,yoda.fines,yoda.arr = mancon(file_path,C3PO.blurr_val,C3PO.canny,C3PO.size,C3PO.dst_val,luke_skywalker.factor,scale)
...
def yoda_undo():
    ...
    scale = int(e2.get())
    #Tool to draw manual contours
    yoda.diameter,yoda.fines,yoda.arr = mancon_undo(file_path,C3PO.blurr_val,C3PO.canny,C3PO.size,C3PO.dst_val,luke_skywalker.factor,scale)
    img2= Image.open(r'C:\Users\USER\Manual contouring\AUTOSAVED_IMAGE_Manual_contouring.jpeg')
    img3=img2.resize((1000, 700), Image.ANTIALIAS)  
...
def yoda_lazzo():
    ...
    scale = int(e2.get())
    #Tool to draw manual contours
    yoda.diameter,yoda.fines,yoda.arr = mancon_lazzo(file_path,C3PO.blurr_val,C3PO.canny,C3PO.size,C3PO.dst_val,luke_skywalker.factor,scale)
    img2= Image.open(r'C:\Users\USER\Manual contouring\AUTOSAVED_IMAGE_Manual_contouring.jpeg')
    img3=img2.resize((1000, 700), Image.ANTIALIAS)  
...
def R2D2():
  ...
  Label(top,text="""Select number of bins for the graph output:""",justify = LEFT,padx = 20).pack()

  #bins selection
  var = IntVar()
  var.set(10)
  scale = Scale( top, variable = var ,from_=1, to=50,orient=HORIZONTAL)
  scale.pack()

  Label(top,text="""Select LSL and USL values:""",justify = LEFT,padx = 20).pack()

  #lsl and usl selection
  lsl = min(yoda.diameter)
  unit_factor = {1:10,2:1,3:0.01}
  slider = Slider(top, width = 400, height = 60, min_val = min(yoda.diameter)*unit_factor[v.get()], max_val = max(yoda.diameter)*unit_factor[v.get()], init_lis = [min(yoda.diameter)*unit_factor[v.get()] ,max(yoda.diameter)*unit_factor[v.get()]], show_value = True)
  slider.pack()

  #Button(top,text="Close",padx=20,pady=20,command=top.destroy()).pack()
  #print('hello')
  top.mainloop()
  #print('hello1')
...
def plot():
	...
	#fine_tot = float(10)
	if fine_tot != float(0):
		s = 'Fines = '+str(round(fine_tot,2))+'% of area with respect to the \n area of all the photos'
		...
		axs[1].text(-0.08, -0.035, s, fontsize =  'x-small')
...
#-----------------------------------------------------------------Page0
canvas = Canvas(page0, width= 900, height=700, bg="black",highlightbackground='black')
canvas.grid(row=0,column=0,padx=325,pady=20)

img2= Image.open(r'C:\Users\USER\NEW BUTTONS 5\tot_v8\Button images\logo_OG.png')
img3=img2.resize((900, 700), Image.ANTIALIAS)  
canvas.image_tk = ImageTk.PhotoImage(img3)
image_id = canvas.create_image(0,0, anchor="nw",image=canvas.image_tk)
canvas.itemconfigure(image_id, image=canvas.image_tk)
...
bt1.grid(row=1,column=0,sticky='n',pady=10)
...
lsl = min(diameter)
unit_factor = {1:10,2:1,3:0.01}
slider98 = Slider(page4, width = 400, height = 60, min_val = 0, max_val = max(diameter)*unit_factor[v.get()], init_lis = [0,max(diameter)*unit_factor[v.get()]], show_value = True)
